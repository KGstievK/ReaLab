...
const getPositionWord = (count: number) => {
...
const ShortlistReaLabSection = () => {
  ...
  const handleAddSelected = async () => {
    ...
    try {
      ...
      setMessage(
        `В список запроса добавлено ${selectedFavorites.length} ${getPositionWord(selectedFavorites.length)}.`,
      );
    } catch (mutationError) {
      const apiError = extractApiErrorInfo(
        mutationError,
  ...
  const handleCompareToggle = (item: FavoriteItem) => {
    const primaryVariant = getPrimaryVariant(item);
    const nextState = toggleCompareItem({
      id: item.clothes.id,
      href: buildProductHref(item.clothes),
      name: item.clothes.clothes_name,
      categoryName: "Saved shortlist",
      imageUrl: item.clothes.clothes_img[0]?.photo || "",
      price: toNumber(item.clothes.price),
      discountPrice: toNumber(item.clothes.discount_price),
      defaultSize: primaryVariant.size,
      defaultColorId: primaryVariant.colorId,
      defaultColorLabel: primaryVariant.colorLabel,
      availabilityLabel: "Под заказ",
    });

    if (nextState === "full") {
      setMessage("В сравнении уже 4 позиции. Откройте compare layer и освободите слот.");
      return;
    }

    setMessage(
      nextState === "added"
        ? `Позиция «${item.clothes.clothes_name}» добавлена в compare layer.`
        : `Позиция «${item.clothes.clothes_name}» удалена из compare layer.`,
    );
  };
    ...
    <section className={scss.SavedShortlistSection}>
      <div className={scss.heroCard}>
        <div className={scss.heroCopy}>
          ...
          <p>
            Собирайте shortlist оборудования, отмечайте ключевые позиции и переводите их в
            request basket, compare layer или RFQ.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className={scss.emptyState}>
          ...
          <p>
            Сохраняйте интересующие системы и позиции, чтобы быстро вернуться к ним при
            подготовке RFQ, compare review или внутреннего согласования.
          </p>
          <div className={scss.emptyActions}>
            <button
              type="button"
              className={scss.emptyPrimary}
              onClick={() => router.push("/catalog")}
            >
              Перейти в каталог
            </button>
            <Link href="/compare" className={scss.emptySecondary}>
              Открыть compare layer
            </Link>
          </div>
        </div>
      ) : (
                ...
                <article key={item.id} className={scss.card}>
                  ...
                  <div className={scss.cardInfo}>
                    ...
                    <div className={scss.cardActions}>
                      ...
                      >
                        <FiShuffle />
                        {compareIds.has(item.clothes.id) ? "В сравнении" : "Сравнить"}
                      </button>
...
export default ShortlistReaLabSection;
